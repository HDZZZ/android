...
import com.research.receiver.PushChatMessage;
import com.research.service.SnsService;
import com.research.service.type.XmppType;
import com.research.widget.MainSearchDialog;
import com.research.widget.MainSearchDialog.OnFinishClick;
import com.research.widget.ResizeLayout;
import com.research.widget.ResizeLayout.OnResizeListener;
...
public class ChatMainActivity  extends BaseActivity implements OnItemLongClickListener, OnClickListener, OnTouchListener, OnPageChangeListener{
	...
	private void initComponent(){
		...
		mReaderImpl = new ReaderImpl(ChatMainActivity.this, handler, audioRecorder){
			...
			public void stop(String path) {
				...
				if (path.equals(mContext.getString(R.string.move_remove))){
					showToast(mContext.getString(R.string.move_remove));
				}
	...
	private void sendText() {
		...
		if (str != null
				&& (str.trim().replaceAll("\r", "")
						...
						.replaceAll("\f", "")) != "") {
			if(str.length() > ResearchCommon.MESSAGE_CONTENT_LEN){
	 ...
	 * 页面销毁
	 * (non-Javadoc)
	 * @see com.research.BaseActivity#onDestroy()
	 */
	@Override
	protected void onDestroy(){
		super.onDestroy();
		ResearchCommon.saveCamerUrl(mContext,"");
	...
	private void addView(final int i){
		...
		gridView.setAdapter(new EmojiAdapter(mContext, mTotalEmotionList.get(i), ResearchCommon.mScreenWidth));
		mViewList.add(view);
	}
	...
	class OnVoice implements OnTouchListener {
		...
		public boolean onTouch(View view, MotionEvent event) {
			...
			case MotionEvent.ACTION_DOWN:
				startY = event.getY();
				audioRecorder.udapteState(AudioRecorderAction.SUCCESS);
			...
			case MotionEvent.ACTION_UP:
				mVoiceSendBtn.setBackgroundResource(R.drawable.login_btn_n);
				Log.i(TAG, "onTouch: event.getRawY()>= mWindowWidth,+(event.getY()-startY)"+Math.abs(event.getY()-startY)+",(mWindowWidth/5)"+(mWindowWidth/5));
				if (Math.abs(event.getY()-startY)>= (mWindowWidth/5) ){
					//Log.i(TAG, "onTouch: event.getRawY()>= mWindowWidth,+(event.getY()-startY)"+(event.getY()-startY)+",(mWindowWidth/5)"+(mWindowWidth/5));
					//mReaderImpl.stop(mContext.getString(R.string.move_remove));
					try {
						audioRecorder.udapteState(AudioRecorderAction.MOVE_REMOVE);
	...
	class MyAdapter extends BaseAdapter{
		...
		private void bindView(final ViewHolder viewHolder, final MessageInfo messageInfo,
							  final long lasttime){
			...
			if(0 == type){
				...
				/*viewHolder.imgMsgVoice.setOnClickListener(new OnClickListener() {
					@Override
					public void onClick(View view) {
						//TODO
						Log.i("555", "onClick: GONE");
		...
		public void onReceive(Context context, Intent intent) {
			...
			if(SnsService.ACTION_CONNECT_CHANGE.equals(action)){
				...
				if(type!=0){
					...
					if(!TextUtils.isEmpty(id)){
						if(!TextUtils.isEmpty(uid) && uid.equals(ResearchCommon.getUserId(mContext))){
							if(id.equals(fCustomerVo.uid)){
								destoryDialog(mContext.getString(R.string.you_have_been_removed_from_group));
							}
						}
	...
	protected void createDialog(Context context, String cardTitle,final int type,final String okTitle) {
		...
		okBtn.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				...
				ResearchCommon.sendMsg(mBaseHandler, BASE_SHOW_PROGRESS_DIALOG, "请求发送中,请稍后...");
		...
		new Thread(){
			public void run() {
				try {
					...
					ResearchJiaState status = ResearchCommon.getResearchInfo().favoreiteMoving(ownerUid, groupid, favoriteContent);
					ResearchCommon.sendMsg(mHandler, GlobalParam.MSG_CHECK_FAVORITE_STATUS,status);
	...
	public void setPhotoImage(Intent data){
		...
		Log.i(TAG, "setPhotoImage: path="+path);
