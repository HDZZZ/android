...
public class LocationActivity extends BaseActivity implements OnClickListener, OnTouchListener  {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		double lng = getIntent().getDoubleExtra("lng", ResearchCommon.getCurrentLng(mContext));
		double lat = getIntent().getDoubleExtra("lat", ResearchCommon.getCurrentLat(mContext));
	...
	private MKSearchListener mMySearchListener = new MKSearchListener() {
		...
		public void onGetAddrResult(MKAddrInfo arg0, int arg1) {
			...
			if(arg0 != null){
				...
				ResearchCommon.sendMsg(mHandler, MSG_SHOW_NEARY_LOCATION, poiList);
			}
		}
		...
		new Thread(){
			public void run() {
				try {
					...
					ResearchJiaState status = ResearchCommon.getResearchInfo().favoreiteMoving(mFuid, mGroupId, favoriteContent);
