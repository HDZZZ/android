...
public class SettingTab extends BaseActivity implements OnClickListener{
		...
		public void onReceive(Context context, Intent intent){
			if(intent != null){
				if(intent.getAction().equals(GlobalParam.SWITCH_LANGUAGE_ACTION)){
					...
					mUser = ResearchCommon.getLoginResult(mContext);
				}
			}
		}
	};
	...
	public void onClick(View v) {
		...
		case R.id.logout://退出登陆
			...
			SharedPreferences preferences = this.getSharedPreferences(ResearchCommon.LOGIN_SHARED, 0);
			Editor editor = preferences.edit();
			editor.remove(ResearchCommon.LOGIN_RESULT);
			editor.commit();
			ResearchCommon.setUid("");
			ResearchCommon.setServer("");
			
			SharedPreferences server = this.getSharedPreferences(ResearchCommon.SERVER_SHARED, 0);
			Editor serverEditor = server.edit();
			serverEditor.remove(ResearchCommon.SERVER_PREFIX);
			serverEditor.commit();
