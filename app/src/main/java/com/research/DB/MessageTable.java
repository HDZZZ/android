 ...
 */
	...
	public void delete(MessageInfo message) {
		mDBStore.delete(TABLE_NAME, COLUMN_TAG + "='" + message.tag + "' AND " + COLUMN_LOGIN_ID + "='" + ResearchCommon.getUserId(BMapApiApp.getInstance()) + "'", null);
	}
	
	public boolean delete(String toId, int type) {
		try {
			if(type == 300){
				mDBStore.delete(TABLE_NAME, COLUMN_TO_ID + "='" + toId + "'" + " AND " + COLUMN_TYPE + "=300" + " AND " + COLUMN_LOGIN_ID + "='" + ResearchCommon.getUserId(BMapApiApp.getInstance()) + "'", null);
			}else if(type == 500){
				mDBStore.delete(TABLE_NAME, COLUMN_TO_ID + "='" + toId + "'" + " AND " + COLUMN_TYPE + "=500" + " AND " + COLUMN_LOGIN_ID + "='" + ResearchCommon.getUserId(BMapApiApp.getInstance()) + "'", null);
			}else {
				mDBStore.delete(TABLE_NAME, (COLUMN_FROM_UID + "='" + toId + "' or " + COLUMN_TO_ID + "='" + toId + "'") + " AND "  + COLUMN_TYPE + "=100" + " AND " + COLUMN_LOGIN_ID + "='" + ResearchCommon.getUserId(BMapApiApp.getInstance()) + "'", null);
			}
			return true;
		} catch (Exception e) {
			e.printStackTrace();
	...
	public boolean updateVoiceContent(String tag, String content){
		...
		try {
			String sql = "UPDATE " + TABLE_NAME + " SET " + COLUMN_CONTENT + "='" + content + "' WHERE " + COLUMN_TAG + "='" + tag + "' AND " + COLUMN_TYPE + "=100 AND " + COLUMN_LOGIN_ID + "='" + ResearchCommon.getUserId(BMapApiApp.getInstance()) + "'";
	...
	public boolean updateReadState(String id, int type){
		...
		try {
			String sql = "";
			if(type == 100){
				sql = "UPDATE " + TABLE_NAME + " SET " + COLUMN_READ_STATE + "=1 WHERE " + (COLUMN_FROM_UID + "='" + id + "' OR " + COLUMN_TO_ID + "='" + id) + "' AND "  + COLUMN_TYPE + "=100 AND " + COLUMN_LOGIN_ID + "='" + ResearchCommon.getUserId(BMapApiApp.getInstance()) + "'";
			}else if(type == 300){
				sql = "UPDATE " + TABLE_NAME + " SET " + COLUMN_READ_STATE + "=1 WHERE " + COLUMN_TO_ID + "='" + id + "' AND " + COLUMN_TYPE + "=300 AND " + COLUMN_LOGIN_ID + "='" + ResearchCommon.getUserId(BMapApiApp.getInstance()) + "'";
			}else if(type == GlobleType.MEETING_CHAT){
				sql = "UPDATE " + TABLE_NAME + " SET " + COLUMN_READ_STATE + "=1 WHERE " + COLUMN_TO_ID + "='" + id + "' AND " + COLUMN_TYPE + "=500 AND " + COLUMN_LOGIN_ID + "='" + ResearchCommon.getUserId(BMapApiApp.getInstance()) + "'";
			}
			mDBStore.execSQL(sql);
			return true;
		} catch (SQLiteConstraintException e) {
	...
	public boolean updatePrivateReadState(String id){
		...
		try {
			String sql = "UPDATE " + TABLE_NAME + " SET " + COLUMN_READ_STATE + "=1 WHERE " + (COLUMN_FROM_UID + "='" + id + "' OR " + COLUMN_TO_ID + "='" + id) + "' AND " + COLUMN_TYPE + "=100 AND " + COLUMN_LOGIN_ID + "='" + ResearchCommon.getUserId(BMapApiApp.getInstance()) + "'";
			mDBStore.execSQL(sql);
			return true;
		} catch (SQLiteConstraintException e) {
			e.printStackTrace();
	...
	public List<MainSearchEntity> queryByContent(String searchContent,String toId,int type){
		...
		try {
			...
			if(type == 0){//查询所有的数据（包括单聊和群聊）
				querySql = "SELECT * FROM " + TABLE_NAME + " WHERE "+ COLUMN_LOGIN_ID + "='" + ResearchCommon.getUserId(BMapApiApp.getInstance()) + "'" 
				...
				if(type == 100 && (toId!=null && !toId.equals(""))){
					querySql = "SELECT * FROM " + TABLE_NAME 
							...
							+ " AND "+COLUMN_LOGIN_ID + "='" + ResearchCommon.getUserId(BMapApiApp.getInstance()) + "'" 
							+" AND " +COLUMN_CONTENT +" like '%"+searchContent+"%' "
							+ " AND "+ COLUMN_MESSAGE_TYPE +" ="+MessageType.TEXT;
	...
	public List<MessageInfo> query(String toId, int autoID, int type){
		...
		try {
			...
			if(type == 100){
				if(autoID == -1){
					querySql = "SELECT rowid, * FROM " + TABLE_NAME + " WHERE (" + COLUMN_FROM_UID + "='" + toId + "' or " + COLUMN_TO_ID + "='" + toId + "')" + " AND " + COLUMN_LOGIN_ID + "='" + ResearchCommon.getUserId(BMapApiApp.getInstance()) + "'" 
							+ " AND " + COLUMN_TYPE + "=" + type +  " ORDER BY rowid" + " DESC LIMIT 0," + ResearchCommon.LOAD_SIZE;
				}else {
					querySql = "SELECT rowid, * FROM " + TABLE_NAME + " WHERE (" + COLUMN_FROM_UID + "='" + toId + "' or " + COLUMN_TO_ID + "='" + toId + "')" + " AND " + COLUMN_LOGIN_ID + "='" + ResearchCommon.getUserId(BMapApiApp.getInstance()) + "'" 
							+ " AND " + " rowid<" + autoID  + " AND " + COLUMN_TYPE + "=" + type +  " ORDER BY rowid" + " DESC LIMIT 0," + ResearchCommon.LOAD_SIZE;
				}
				
			}else {
				if(autoID == -1){
					querySql = "SELECT rowid, * FROM " + TABLE_NAME + " WHERE " + COLUMN_TO_ID + "='" + toId + "' AND " + COLUMN_LOGIN_ID + "='" + ResearchCommon.getUserId(BMapApiApp.getInstance()) + "'" 
							+ " AND " + COLUMN_TYPE + "=" + type +  " ORDER BY rowid" + " DESC LIMIT 0," + ResearchCommon.LOAD_SIZE;
				}else {
					querySql = "SELECT rowid, * FROM " + TABLE_NAME + " WHERE " + COLUMN_TO_ID + "='" + toId + "' AND " + COLUMN_LOGIN_ID + "='" + ResearchCommon.getUserId(BMapApiApp.getInstance()) + "'" 
							+ " AND " + " rowid<" + autoID  + " AND " + COLUMN_TYPE + "=" + type + " ORDER BY rowid" + " DESC LIMIT 0," + ResearchCommon.LOAD_SIZE;
	...
	public int queryUnreadCountByID(String id, int type){
		...
		try {
			...
			if(type == 100){
				...
				+ "' AND " + COLUMN_TYPE + "=100 AND " + COLUMN_READ_STATE + "=0" + " AND " + COLUMN_LOGIN_ID + "='" + ResearchCommon.getUserId(BMapApiApp.getInstance()) + "'" 
						+ " ORDER BY " + COLUMN_SEND_TIME + " DESC ";
			}else {
				querySql = "SELECT rowid, * FROM " + TABLE_NAME + " WHERE " + COLUMN_TO_ID + "='" + id 
						+ "' AND " + COLUMN_TYPE + "=type AND " + COLUMN_READ_STATE + "=0" + " AND " + COLUMN_LOGIN_ID + "='" + ResearchCommon.getUserId(BMapApiApp.getInstance()) + "'" 
								+ " ORDER BY " + COLUMN_SEND_TIME + " DESC ";
			}
	...
	public int queryUnreadCount(){
		...
		try {
			...
			String querySql = "";
			querySql = "SELECT rowid, * FROM " + TABLE_NAME + " WHERE " + COLUMN_READ_STATE + "=0" + " AND " + COLUMN_LOGIN_ID + "='" + ResearchCommon.getUserId(BMapApiApp.getInstance()) + "'";
