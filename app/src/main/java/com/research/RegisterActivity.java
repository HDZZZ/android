...
public class RegisterActivity extends BaseActivity implements OnClickListener {
				...
				if(register.mState!=null){
					if(register.mState.code == 0){
						if (register.mLogin != null) {
							...
							ResearchCommon.saveLoginResult(mContext, register.mLogin);
							ResearchCommon.setUid(register.mLogin.uid);
						}

						Editor editor = mContext.getSharedPreferences(ResearchCommon.REMENBER_SHARED, 0).edit();
	...
	private void register(){
		...
		new Thread(){
			public void run() {
				try {
					ResearchCommon.sendMsg(mBaseHandler, BASE_SHOW_PROGRESS_DIALOG,R.string.commit_dataing);
					//++检测验证码++//
					ResearchJiaState validState = ResearchCommon.getResearchInfo().checkVerCode(mInputNumber, mInputValidCode);
					//-检测验证码-//
					
					if(validState !=null && validState.code == 0){//注册
						LoginResult status = ResearchCommon.getResearchInfo().register(mInputNumber, 
								mInputPwd,mInputValidCode);
						ResearchCommon.sendMsg(mHandler, GlobalParam.REGISTER_REQUEST, status);
					}else{
						ResearchCommon.sendMsg(mHandler, GlobalParam.MSG_CHECK_VALID_ERROR, validState);
					}
		...
		new Thread() {
			public void run() {
				try {
					...
					ResearchJiaState status = ResearchCommon.getResearchInfo().getVerCode(mInputNumber,type);
				
					ResearchCommon.sendMsg(mHandler, GlobalParam.MSG_CHECK_STATE,status);
					mBaseHandler.sendEmptyMessage(BASE_HIDE_PROGRESS_DIALOG);
				} catch (ResearchException e) {
					e.printStackTrace();
					ResearchCommon.sendMsg(mBaseHandler, BASE_MSG_TIMEOUT_ERROR,
