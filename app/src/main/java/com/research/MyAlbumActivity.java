...
import com.research.adapter.MyAlbumAdpater;
...
import com.research.widget.PopWindows;
import com.research.widget.MyPullToRefreshListView.OnChangeStateListener;
import com.research.widget.PopWindows.PopWindowsInterface;
import com.research.widget.RoundImageView;
...
public class MyAlbumActivity extends BaseActivity implements OnClickListener, RecyclerListener, OnChangeStateListener{
	...
	private Handler mHandler = new Handler(){
		...
		public void handleMessage(Message msg) {
			...
			case GlobalParam.MSG_UPLOAD_STATUS:
				ResearchJiaState returnStatus = (ResearchJiaState)msg.obj;
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		if(mToUserID == null || mToUserID.equals("")){
			mToUserID = ResearchCommon.getUserId(mContext);
		}
	...
	private void registerReceiver(){
		...
		filter.addAction(GlobalParam.ACTION_LINKE_WEIBO);
		filter.addAction(FriensLoopActivity.MSG_REFRESH_MOVIINF);
	...
	private BroadcastReceiver broadcaset = new BroadcastReceiver() {
		...
		public void onReceive(Context context, Intent intent) {
			...
			}else if(action.equals(FriensLoopActivity.MSG_REFRESH_MOVIINF)){
				if(mMyAlbum!=null){
					mMyAlbum = null;
				}
				getLoopData(GlobalParam.LIST_LOAD_FIRST);
	...
	private void initCompent(){
		if(mToUserID.equals(ResearchCommon.getUserId(mContext))){
		...
		if (mListView.getHeaderViewsCount()==0) {
			...
			if(mToUserID!=null && !mToUserID.equals("") && mToUserID.equals(ResearchCommon.getUserId(mContext))){
			...
			public void run() {
				try {
					if (loadType == GlobalParam.LIST_LOAD_FIRST) {
						...
						LoginResult loginResult = ResearchCommon.getResearchInfo().getUserInfo(mToUserID);
					...
					if (isExitsData) {
						...
						mMyAlbum = ResearchCommon.getResearchInfo().myHomeList(page,mToUserID);
