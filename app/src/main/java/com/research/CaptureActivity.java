...
import com.research.camera.CameraManager;
import com.research.decoding.CaptureActivityHandler;
import com.research.decoding.InactivityTimer;
...
import com.research.widget.ViewfinderView;
/**
 * 扫一扫
 * @author 
 */
		...
		new Thread(){
			...
			public void run(){
				if(ResearchCommon.verifyNetwork(mContext)){
					try {
						Room result = ResearchCommon.getResearchInfo().getRommInfoById(groupId);
						if(result != null && result.state != null && result.state.code == 0){

							SQLiteDatabase db = DBHelper.getInstance(mContext).getWritableDatabase();
							RoomTable roomTab = new RoomTable(db);
							roomTab.insert(result);
							if((ResearchCommon.getUserId(mContext)!=null
									&& !ResearchCommon.getUserId(mContext).equals(""))){
								if(result.isjoin ==1){
